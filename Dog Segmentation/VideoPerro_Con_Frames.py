...
def calculate_behavior(body_posture, tail_speed):
    ...
    if tail_speed > 130 and body_posture == "Tenso":
        ...
        return "Comportamiento neutral."  # Cambiado de "indefinido" a "neutral"
...
def predict_video(video_path, conf_threshold, iou_threshold):
    """Procesa un video para predecir la dirección y estado del perro cada 5 fotogramas."""
    ...
    behaviors = []  # Lista para almacenar comportamientos
    ...
    while cap.isOpened():
        ...
        if frame_count % 5 == 0:  # Solo procesar cada 5 fotogramas
            for r in results:
                frame = r.plot()
                ...
                if bounding_boxes:
                    ...
                    # Guardar el comportamiento en la lista
            ...
            # Calcular la media de comportamientos
            if behaviors:
                most_common_behavior = Counter(behaviors).most_common(1)[0][0]
            else:
                most_common_behavior = "No se detectó comportamiento"

            final_message = f"Velocidad de la cola: {tail_speed:.2f} píxeles/segundo. {speed_description} El comportamiento mayoritario es: {most_common_behavior}"

            yield pil_img, final_message
            behaviors.clear()  # Limpiar comportamientos para la próxima serie de 5 fotogramas
