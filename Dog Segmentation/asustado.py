...
# Cargar el modelo YOLO preentrenado en la GPU
...
def analyze_pupil(image, box):
    ...
    black_threshold = 50  # Cambia este valor si es necesario
...
def categorize_tail_speed(speed):
    ...
    if speed > 130:
        return "Altos (Ej. 130 píxeles/segundo): Indican que la cola se está moviendo rápidamente. Esto podría ser un signo de excitación o agitación del perro."
    elif speed > 25:
        return "Moderados (Ej. 25 píxeles/segundo): Pueden indicar un movimiento moderado de la cola, posiblemente asociado con calma o interés."
    elif speed > 5:
        return "Bajos (Ej. 5 píxeles/segundo): Sugieren que la cola se está moviendo lentamente, lo que podría ser una señal de tranquilidad o relajación."
    else:
...
def calculate_direction_and_state(image, bounding_boxes):
    ...
    # Determinar la dirección del perro
    ...
    if ear_boxes:
        ...
        if tail_box:
            ...
            tail_up = True  # Se asume que la cola está arriba si no se detecta caja de cola

            # Determinar el estado del perro
            if ears_above_line and tail_up:
                behavior_state = "Contento"
            elif not ears_above_line and tail_up:
                behavior_state = "Inquieto"
            elif not ears_above_line and not tail_up:
                behavior_state = "Asustado"
            else:
                behavior_state = "Cauteloso"
            ...
            # Si no se detecta la caja de la cola, verificar el estado de las orejas
            if not ear_boxes:
                final_message = "No se detectaron orejas suficientes para determinar el comportamiento."
            else:
                ear_centroids = [(x1 + w / 2, y1 + h / 2) for x1, y1, w, h in ear_boxes]
                ears_above_line = any([centroid[1] < image.size[1] / 2 for centroid in ear_centroids])
                if ears_above_line:
                    final_message = "Las orejas están abajo. El perro está asustado."
                else:
                    final_message = "No se detectó la caja de la cola para determinar la posición."
